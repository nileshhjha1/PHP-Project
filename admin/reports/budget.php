...
if(isset($_SESSION['userdata']['id']) && $_SESSION['userdata']['id'] ) {
    ...
    // echo "User ID not found or not equal to 17";
}
?>
        ...
        <div id="printable">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    $i = 1;
                    $total = 0;
                        $qry = $conn->query("SELECT r.*,c.category,c.balance from `running_balance` r inner join `categories` c on r.category_id = c.id where c.status=1 and r.user_id = $user_id and r.balance_type = 1 and date(r.date_created) between '{$date_start}' and '{$date_end}' order by unix_timestamp(r.date_created) asc");
